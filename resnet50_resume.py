...
RESUME                   = True                 # set True to continue
RESUME_PATH              = MODEL_SAVE_PATH      # where to load from
RESUME_COMPLETED_EPOCH   = 100                  # last completed epoch previously
# =============================
...
def main():
    ...
    if RESUME and os.path.isfile(RESUME_PATH):
        ...
        try:
            ...
            print(f"[RESUME] Optimizer state mismatch, reinitializing optimizer. Detail: {e}")
        if "epoch" in ckpt:
            start_epoch = int(ckpt["epoch"])
        else:
            start_epoch = int(RESUME_COMPLETED_EPOCH)
        best_acc = float(ckpt.get("val_acc", 0.0))
        print(f"[RESUME] Completed epoch in ckpt: {start_epoch}, best_val_acc so far: {best_acc*100:.2f}%")
    elif RESUME:
        print(f"[RESUME] Requested, but checkpoint not found at {RESUME_PATH}. Starting fresh.")

    # Skip LR finder if resuming
    run_lrf = RUN_LR_FINDER and not RESUME
    if run_lrf:
    ...
    # Continue scheduler smoothly when resuming
    ...
    # If completed 100, this starts at 101
    for epoch in range(start_epoch + 1, EPOCHS + 1):
        ...
        # Save best model (keep best across both phases)
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save({
            ...
            print(f"[CKPT] New best model saved at epoch {epoch} with val_acc={best_acc*100:.2f}%")
